...
Hashing is one-way; it is not [encryption](./engineering-foundations-encryption.md) without a decryption key. Because the possible input space is larger than the fixed output space, collisions must exist mathematically. Security depends on them being impractical to find.
